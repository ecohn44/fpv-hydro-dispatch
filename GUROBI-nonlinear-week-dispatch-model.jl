...
S0 = V[1]   # initial reservoir conditions [~6.3e10 m3]
...
# @constraint(model, ReleaseEnergy[t=1:T*N_sim], ph[t] <= (eta*rho_w*g*u[t]*(a*(S[t]^b)))/(1e6)) # new hydraulic head function
# @constraint(model, ReleaseEnergy[t=1:T*N_sim], ph[t] <= (eta*rho_w*g*u[t]*(S[t]/SA))/(1e6)) # old hydraulic head function
@constraint(model, ReleaseEnergy[t=1:T*N_sim], ph[t] <= hpcap[t]) # deterministic capacity 
...
# ---------- DUAL VALUES -------- # 
print(dual_status(model)) # returns the dual status for each time t
print(shadow_price(Water_Contract)) # returns 0 when the constraint is nonbinding
...
# Generation Plots
# hpcap = (eta*rho_w*g*u_s.*(S_s/SA))/(1e6) # hydraulic head capacity on hydropower generation, convert to MWh
...
# df = DataFrame(Column1 = vec(hpcap))
# CSV.write("data/hpcap.csv", df)